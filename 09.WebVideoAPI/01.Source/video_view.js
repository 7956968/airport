...
const RequestTypeRealtime = 1
const RequestTypeHistory = 2
var mIP = "";
var mPort = "";
var mUserName = "";
var mPwd = "";
var mVehicleNo = "";
var mVideoCount = 0;
var videoType = RequestTypeRealtime
function VideoView(videoCount, type) {
    videoType = type
...
VideoView.prototype.init = function (ip, port, username, pwd, vehicleNo, startTime, endTime) {
    mIP = ip;
    mPort = port;
    mPwd = pwd;
    mUserName = username;
    ...
    $('#startListen').click(function () {
        ...
        vsclientSession.startListening(mVehicleNo, 1);
        setButtunState("#startListen", "disabled");
        setButtunState("#stopListen", "enable");
    });
    $('#stopListen').click(function () {
        console.log("stopAudio");
        vsclientSession.stopListening();
        setButtunState("#startListen", "enable");
        setButtunState("#stopListen", "disabled");
    });
...
//得到录像列表数据
function getList(ReplayList) {
    for (var k = 0; k < ReplayList.length; k++) {
        var RepList = ReplayList[k];
        if (RepList.channel >= 0) {
            var typeAlarm = (RepList.alarm_type || RepList.alarm_typeex) ? "报警录像" : "常规录像"
            $('<p><a href="#">' + changeTime(RepList.begin_time) + '-' + changeTime(RepList.end_time) +
                '--' + typeAlarm + '</a></p>').appendTo('.list').on('click', (function (recEntry) {
                    var recordInfo = jQuery.extend(true, {}, recEntry);
                    return function () {
                        $('#replay_api').show()
                        var recordInfo1 = jQuery.extend(true, {}, recordInfo);
                        vsclientSession.stopReplay($("#replay_api")[0])
                        vsclientSession.startReplay(22, 1, $("#replay_api")[0], recordInfo1);
                        $("#show_status").text("正在获取视频..");
                    };
                }(RepList)))

        }
    }
    $("#show_status").text("获取视频列表成功");
}
//为获取录像列表数据,发出请求
...
function connectNet() {
    var callback = {
        ...
        onlogon: function () {
            ...
            if (videoType == RequestTypeHistory) {
                getListData(front.id)
            }
